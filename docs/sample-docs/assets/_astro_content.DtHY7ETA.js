...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content.DM5DLZ-1.js');
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets.FljGAAvJ.js').then(n => n._);
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules.CkZoKwyx.js');
